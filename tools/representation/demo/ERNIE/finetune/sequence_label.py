...
def create_model(args, pyreader_name, ernie_config, is_prediction=False):
    ...
    (src_ids, sent_ids, pos_ids, task_ids, input_mask, labels, seq_lens) = fluid.layers.read_file(pyreader)
    ...
    ce_loss, probs = fluid.layers.softmax_with_cross_entropy(logits=fluid.layers.flatten(logits, axis=2),
                                                             label=labels,
                                                             return_softmax=True)
    ...
    graph_vars = {"loss": loss, "probs": probs, "labels": ret_labels, "infers": ret_infers, "seq_lens": seq_lens}
...
def evaluate(exe, program, pyreader, graph_vars, tag_num, eval_phase, dev_count=1):
    fetch_list = [graph_vars["labels"].name, graph_vars["infers"].name, graph_vars["seq_lens"].name]
    ...
    if eval_phase == "train":
        ...
        rets = {"precision": precision, "recall": recall, "f1": f1, "loss": np.mean(np_loss)}
        if "learning_rate" in graph_vars:
